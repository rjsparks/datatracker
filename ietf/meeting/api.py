...
def system_person():
    """The Person that authors records pushed by an application rather than a human

    These endpoints authenticate an application by token, so there is no requesting
    person to attribute a document event to.
    """
    return Person.objects.get(name="(System)")
...
class SessionVideoUrlView(SessionDataView):
    ...
    def post(self, request, session_id):
        ...
        return self.applied(
            session, save_session_video_url(session, data["url"], system_person())
        )
...
class SessionChatlogView(SessionDataView):
    ...
    def post(self, request, session_id):
        ...
        return self.applied(
            session,
            save_session_json_doc(
                session, "chatlog", data["chatlog"], system_person()
            ),
        )
...
class SessionPollsView(SessionDataView):
    ...
    def post(self, request, session_id):
        ...
        return self.applied(
            session,
            save_session_json_doc(session, "polls", data["polls"], system_person()),
        )
