...
def save_bluesheet(session, file, by, encoding='utf-8', request=None):
...
def generate_bluesheet(session, by, request=None):
    ...
    return save_bluesheet(
        session,
        ContentFile(text.encode("utf-8"), name="unusednamepartsothereisanextension.txt"),
        by,
        request=request,
    )
...
def finalize(request, meeting):
    ...
    for session in meeting.session_set.all():
        ...
        if int(meeting.number) >= 108:
            save_error = generate_bluesheet(session, request.user.person, request=request)
            if save_error:
