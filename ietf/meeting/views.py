...
def upload_session_bluesheets(request, session_id, num):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            save_error = save_bluesheet(session, file, request.user.person, encoding=form.file_encoding[file.name], request=request)
            if save_error:
...
def api_upload_bluesheet(request):
    ...
    with open(name, "br") as file:
        save_err = save_bluesheet(session, file, request.user.person, request=request)
    if save_err:
